...
-- [回答]
SELECT store_name 店舗名, goods_name 商品名, quantity 在庫数 from stock_table 
join store_table 
...
-- [回答]
INSERT INTO goods_table VALUES ('M001','マフラー',4500,'2020-08-18')

-- 7. 在庫テーブルの商品コード='S987'、かつ、店舗コード='EA01'に対して、「在庫数=10、更新日付=本日日付」で更新しなさい。※実行後のSELECT結果も貼付すること。
-- [回答]
UPDATE stock_table SET quantity = 10, update_day = '2020-08-18' WHERE goods_code = 'S987' AND store_code = 'EA01'
